...
const getColorForState = (state: VisualState) => {
  switch (state) {
    ...
    case "outOfBounds": 
      return "bg-bg-main border-border-default text-text-secondary opacity-50 grayscale";
    default: 
...
export default function SearchingCanvas() {
  ...
  return (
    <div className="flex flex-col items-center w-full space-y-8">
      ...
      {/* The Visual Canvas */}
      <div className="flex flex-col items-center justify-center w-full max-w-4xl p-8 bg-bg-main relative overflow-x-auto rounded-card border-2 border-dashed border-border-default shadow-inner min-h-[420px] shrink-0 transition-all duration-300">
        
        <div className="mb-12 flex flex-col items-center animate-in fade-in zoom-in">
          <span className="text-xs font-bold text-text-placeholder mb-2 uppercase tracking-widest">
            Target Value
          </span>
          <div className="w-16 h-16 flex items-center justify-center text-2xl font-bold bg-amber-50 border-2 border-amber-400 text-amber-800 rounded-btn shadow-sm">
            {targetValue}
          </div>
        </div>
          ...
          {renderArray.map((item, index) => {
            ...
            return (
              <div key={item.id} className="flex flex-col items-center group relative mt-4">
                ...
                {/* Array Box */}
                <div 
                  className={`w-14 h-14 flex items-center justify-center text-xl font-bold border-2 rounded-btn transition-all duration-300 shadow-sm
                    ${getColorForState(item.state)}
                    ${(isCurrent || isMid) ? 'scale-110 shadow-premium z-10 border-primary' : ''}
                  `}
                >
                  {item.value}
                </div>
                ...
                {/* Index Number */}
                <div className="text-xs text-text-placeholder font-mono mt-3 font-semibold">
                  {index}
                </div>
      ...
      {!isPlaybackMode ? (
        <div className="flex flex-col items-center space-y-6 w-full max-w-2xl">
          ...
          <div className="flex w-full space-x-4">
            {/* Custom Input Field */}
            <div className="flex flex-1 items-center space-x-2 bg-bg-card p-2 rounded-btn border border-border-default shadow-sm">
              <input 
                type="text" 
                placeholder={`Array: 12, 24, 35...`}
                value={customInput}
                onChange={(e) => setCustomInput(e.target.value)}
                className="flex-1 px-4 py-2 outline-none text-text-heading font-medium bg-transparent min-w-[150px]"
            ...
            {/* Target Input Field */}
            <div className="flex items-center space-x-2 bg-bg-card p-2 rounded-btn border border-border-default shadow-sm w-48">
              <span className="text-text-placeholder font-semibold pl-2">Target:</span>
              <input 
                type="number" 
                value={targetValue}
                onChange={(e) => setTargetValue(parseInt(e.target.value) || 0)}
                className="flex-1 w-full px-2 py-2 outline-none text-text-heading font-bold bg-transparent text-center"
              />
            </div>
          </div>
